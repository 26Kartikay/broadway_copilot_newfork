...
import { prismaProductToBroadwayShape } from './productDbAdapters';
...
/**
 * Same tagging model as `runProductAutomation` (OpenAI vision + JSON via tagExtractor),
 * but **no Broadway HTTP fetch** — uses name / brand / description / imageUrl already on Product
 * (e.g. from CSV bulk seed). Writes the same Product columns + embedding as the API pipeline.
 */
async function processOneOpenAiTags(
...
export async function runOpenAiTagAndEmbedPipeline(options?: {
  maxProducts?: number;
  concurrency?: number;
  batchSize?: number;
  ...
  const run = await prisma.automationRun.create({
    data: { id: runId, name: 'ProductTaggerOpenAiBulk', startTime, status: 'running' },
  });

  logger.info(
    { runId, maxProducts, concurrency, batchSize, skipEmbedded },
    '[Automation] Starting OpenAI tag + embed bulk (no Broadway API)',
  );

  const products = await getAutomationProductQueue(maxProducts, skipEmbedded);

  if (products.length === 0) {
    logger.info('[Automation] No products to process (OpenAI bulk)');
  ...
  const tasks = products.map((p) => () =>
    processOneOpenAiTags(p, automationConfig.timeoutMs),
  );
  ...
  logger.info(
    ...
    '[Automation] OpenAI bulk tag+embed complete',
  );
