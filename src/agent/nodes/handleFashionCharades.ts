...
async function startNewCharadesRound(state: GraphState): Promise<GraphState> {
  ...
  const systemPromptText = await loadPrompt('handlers/quiz/fashion_charades_generation.txt', state.user);
...
async function evaluateCharadesGuess(state: GraphState, userGuess: string): Promise<GraphState> {
  ...
  const systemPromptText = await loadPrompt('handlers/quiz/fashion_charades_scoring.txt', state.user);
