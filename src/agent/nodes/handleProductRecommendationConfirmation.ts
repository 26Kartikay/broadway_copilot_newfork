...
export async function handleProductRecommendationConfirmation(state: GraphState): Promise<GraphState> {
  ...
  try {
    ...
    if (allProducts.length > 0) {
        ...
        if (replies.length === 0) {
            replies.push({
                reply_type: 'text',
                reply_text: "I couldn't find any specific recommendations at the moment, but I'll keep an eye out!",
            });
        }
    }
    ...
    return {
      ...
      return { 
          ...
          assistantReply: [
              {
                  reply_type: 'text',
                  reply_text: "I had a little trouble finding products right now, but let's try again later!",
              },
              ...getMainMenuReply()
          ], 
          pending: PendingType.NONE 
