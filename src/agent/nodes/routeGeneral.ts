...
export async function routeGeneral(state: GraphState): Promise<GraphState> {
  ...
  if (buttonPayload) {
    if (buttonPayload === 'main_menu' || buttonPayload === 'refresh_conversation_starters') {
      logger.debug(
        { userId },
        `General intent routed to "menu" by button payload: ${buttonPayload}`,
      );
    ...
    if (buttonPayload.startsWith('conversation_starter_')) {
      ...
      const originalStarterText =
        extractedText.charAt(0).toUpperCase() + extractedText.slice(1) + '?'; // Capitalize first letter and add '?'

      logger.debug(
        { userId, originalStarterText },
        'Conversation starter button clicked, routing to chat.',
      );
      ...
      if (state.conversationHistoryTextOnly.length > 0) {
        const lastMessage =
          state.conversationHistoryTextOnly[state.conversationHistoryTextOnly.length - 1];
        if (lastMessage) {
      ...
      if (state.conversationHistoryWithImages.length > 0) {
        const lastMessage =
          state.conversationHistoryWithImages[state.conversationHistoryWithImages.length - 1];
        if (lastMessage) {
          lastMessage.content = [{ type: 'text', text: originalStarterText }];
        }
      }
  ...
  try {
    ...
    try {
      ...
      // If structured output parsing/extraction fails, gracefully fall back to chat intent
      if (
        msg.includes('Failed to parse structured output as JSON') ||
        msg.includes('Failed to extract JSON from model output')
      ) {
        logger.warn({ userId, error: msg }, 'Structured routing failed, defaulting to chat intent');
        return { ...state, generalIntent: 'chat' as GeneralIntent };
      }
